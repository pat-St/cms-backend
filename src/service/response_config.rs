...
fn get_header() -> Response<'static> {
    Response::build()
    .raw_header_adjoin("Access-Control-Allow-Origin", "*")
...
impl<'r, T> ResponseWithHeader<T> where T: serde::Serialize {
    fn response_option(payload: Option<T>) -> response::Result<'r> {
        match payload {
            Some(value) => {
                Response::build()
            ...
            None => Response::build().join(get_header()).status(Status::UnprocessableEntity).ok()
        }
    }
    // TODO: return other type (Response)
    fn response_status(payload: Option<T>) -> response::Result<'r> {
        match payload {
            Some(_) =>  Response::build().join(get_header()).ok(),
            None => Response::build().join(get_header()).status(Status::UnprocessableEntity).ok()
        }
    }
}
...
impl<'r, T> Responder<'r> for ResponseWithHeader<T> where T: serde::Serialize {
    fn respond_to(self, req: &Request) -> response::Result<'r> {
        match req.method() {
            Method::Post | Method::Put | Method::Delete => { Self::response_status(self.0) }
            Method::Get => { Self::response_option(self.0) }
            Method::Options => { Self::response_status(self.0) }
